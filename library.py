...
# import sys
import sys
sys.path.append('/data/scratch/tamarott/neuron-descriptions/src/deps/netdissect/')
# sys.path.append('/data/scratch/schwett/neuron-descriptions/src/deps/netdissect/')
# sys.path.remove(['../../netdissect/'])
sys.path.append('/data/scratch/tamarott/MLIM/synthetic-neurons-dataset/')
sys.path.append('/data/scratch/tamarott/MLIM/synthetic-neurons-dataset/Grounded-Segment-Anything/')
# sys.path.append('/data/scratch/tamarott/MLIM/netdissect/')
# sys.path.append('/data/scratch/tamarott/MLIM/')
# from netdissect import imgviz
sys.path.append('/data/scratch/tamarott/neuron-descriptions/src/deps/')
from netdissect.imgviz import ImageVisualizer #correct the import
    ...
    def call_neuron(self, image_list: List[torch.Tensor])->Tuple[List[int], List[str]]:
        ...
        for image in image_list:
            if  image==None: #for dalle
                ...
                masked_images_list.append(masked_image)
                # ind = np.argmax(acts)
                # masked_image = generate_masked_image(image[ind], masks[ind], "./temp_synthetic.png", self.threshold)
        return activation_list,masked_images_list
...
class Tools:
    ...
    def save_experiment_log(self, activation_list: List[int], image_list: List[str], image_titles: List[str], image_textual_information: List[str] = None):
        ...
        for ind,act in enumerate(activation_list):
            ...
            output.append({"type": "image_url", "image_url": {'url': "data:image/jpeg;base64," + image_list[ind]}}) #gemini
    ...
    def update_experiment_log(self, role, content=None, type=None, type_content=None):
        openai_role = {'execution':'user','maia':'assistent','user':'user','system':'system'}
        if type == None:
            ...
            if type == 'image_url':
                # self.experiment_log.append({'role': role, 'content': [{"type":type, "image_url": type_content}]}) #gemini
                self.experiment_log.append({'role': openai_role[role], 'content': {"type":type, "image_url": {'url': type_content}}}) 
    ...
    def summarize_images(self, image_list: List[str]) -> str:
        ...
        description = ask_model('gpt-4-vision-preview',history)
    ...
    def generate_html(self,name="experiment.html"):
        ...
        # don't plot system+user prompts
        ...
        for entry in self.experiment_log[2:]:
            ...
            if entry['role'] == 'assistant':
                ...
                html_string += f"<pre>{entry['content']['text']}</pre><br>"#gemini
                ...
                for content_entry in entry['content']:
                    ...
                    if "image_url" in content_entry["type"]:
                        # embed()
                        # html_string += f'''<img src="{content_entry['image_url']}"/>''' #gemini
